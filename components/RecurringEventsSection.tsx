...
export function RecurringEventsSection({ events, onEdit, onDelete }: RecurringEventsSectionProps) {
  ...
  return (
    <section className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {events.map((event) => (
          <RecurringEventCard key={event.id} event={event} onEdit={onEdit} onDelete={onDelete} />
        ))}
      </div>
    </section>
  );
